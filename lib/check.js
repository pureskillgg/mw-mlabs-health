...
import { has, isNil, isNotNil } from '@pureskillgg/phi'
...
const withTimeout = (ms, promise) =>
  new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => reject(new Error(err)), ms)
    promise.then(
      (value) => {
        clearTimeout(timer)
        resolve(value)
 ...
 */
export const createSyncCheck =
  (target) =>
  (...args) => {
    if (typeof target !== 'function') {
      throw new Error(
        `Cannot create sync health function from ${typeof target}.`
      )
    }
